...
def cross_val_prediction_helper(data, vars, target, lipid_type):
    ...
    min_scale = y if (y.min() < predictions.min()) else predictions
    plt.plot([min_scale.min(), min_scale.max()], [min_scale.min(), min_scale.max()], 'k--', lw=2)
    ...
    legend1 = plt.legend(list(zip(colors, markers_types)), markers_labels,
            handler_map={tuple: MarkerHandler()}, loc=9)
